...
-- creating table main_menu_dogs

CREATE TABLE IF NOT EXISTS main_menu_dogs
(
    id        BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
...
-- creating table main_menu_cats

CREATE TABLE IF NOT EXISTS main_menu_cats
(
    id        BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    button    TEXT UNIQUE NOT NULL,
    call_back TEXT UNIQUE NOT NULL
...
-- creating table shelter_choosing_menu

CREATE TABLE IF NOT EXISTS shelter_choosing_menu
(
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    button TEXT UNIQUE NOT NULL,
    call_back TEXT UNIQUE NOT NULL
...
ALTER TABLE pets
    ADD COLUMN potential_owner_id BIGINT
...
ALTER TABLE shelters
    ADD COLUMN pet_id BIGINT
        CONSTRAINT fk_pet_id REFERENCES pets (id);

ALTER TABLE shelters
    ADD COLUMN cynologist_id BIGINT
        CONSTRAINT fk_cynologist_id REFERENCES cynologists (id);

ALTER TABLE shelters
    ADD COLUMN location_id BIGINT
        CONSTRAINT fk_location_id REFERENCES locations (id);

ALTER TABLE shelters
    ADD COLUMN recommendations_id BIGINT
...
-- add some data into table main_menu_dogs

INSERT INTO main_menu_dogs
    (button, call_back)
VALUES ('Прислать отчет о питомце', 'Прислать отчет о питомце'),
       ...
       ('Рекомендуемые кинологи', 'Рекомендуемые кинологи'),
       ('Первичные советы', 'Первичные советы'),
       ('Позвать волонтера', 'Позвать волонтера');

-- add some data into table main_menu_cats

INSERT INTO main_menu_cats
    (button, call_back)
VALUES ('Прислать отчет о питомце', 'Прислать отчет о питомце'),
       ('Как взять кошку из приюта', 'Как взять кошку из приюта'),
       ('Узнать информацию о приюте', 'Узнать информацию о приюте'),
       ('Рекомендуемые кинологи', 'Рекомендуемые кинологи'),
       ('Первичные советы', 'Первичные советы'),
       ('Позвать волонтера', 'Позвать волонтера');

-- add some data into table shelter_choosing_menu

INSERT INTO shelter_choosing_menu
(button, call_back)
VALUES ('Меню приюта собак', 'Меню приюта собак'),
       ('Меню приюта кошек', 'Меню приюта кошек');
...
(
    ...
    photo_id     TEXT,
    file_size    BIGINT,
    day_of_month TIMESTAMP WITHOUT TIME ZONE,
    report_text  TEXT,
    media_type   TEXT
);
-- add some data into table reports

INSERT INTO reports (photo_id, file_size, day_of_month, report_text, media_type)
